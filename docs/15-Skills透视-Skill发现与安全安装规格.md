...
FanBox 用户已经可以在 Skills 透视中查看本机 Skill 原件、管理各 Agent 接入和收编原件仓外的本机拷贝，但无法在同一处发现新的外部 Skill。用户目前必须离开 FanBox，自行寻找来源、判断目录结构、下载文件、选择接入对象并处理冲突；这个过程既繁琐，也容易把名称相同但来源不同的 Skill 当成更新，或在未固定版本、未检查文件结构的情况下把不确定内容写入原件仓。

用户需要一个内容丰富、实现简单且安全边界清楚的“发现”能力：能够搜索 skills.sh 的社区索引，一键发起检查与安装，同时始终区分尚未安装的 Skill 条目和本机原件。FanBox 应固定公开 GitHub 来源的具体版本，只对可客观验证的文件与结构风险作判断，不执行外部内容、不声称 Skill 安全，并在失败、冲突或并发变化时保持本机原件可恢复。
...
普通检查通过时，用户确认接入对象（默认全选四个 Agent，允许全不选即纯库存）后即可安装：FanBox 将固定版本内容创建为原件仓 `~/.agents/skills` 中的原件，并按所选 Agent 建立接入（Claude 逐 skill 相对软链、Codex 与 ZCode 配置开关、WorkBuddy 拷贝，见 ADR 0005）。存在脚本、高权限工具或未知许可证等未知风险时，用户必须展开风险明细并勾选确认。FanBox 不接入第三方审计、不执行脚本、不安装依赖、不调用 Agent 分析远端内容。真正落盘复用现有 Skills 写操作的串行、临时副本、原子换位、废纸篓和回滚边界；成功后记录来源身份与固定版本，刷新“已安装”页并展开新原件。同源更新只替换原件一次（原子换位、旧原件进废纸篓可恢复、全部接入即时生效），执行前必须呈现受影响的接入范围。
...
2. As a FanBox user, I want Installed and Discover to be separate top-level tabs, so that I never confuse an external Skill entry with a local Skill original.
...
8. As a privacy-conscious user, I do not want my local Skill list, project path, connection selection or installation history sent with a search, so that discovery exposes only the query required by skills.sh.
...
55. As a user, I want the installation confirmation to show name, description, source, author, fixed commit, connection selection and local check result, so that I know exactly what is being installed.
...
65. As a user, I want to choose any combination of Claude, Codex, ZCode and WorkBuddy as connection targets when installing, so that the discovered Skill reaches the tools I use.
66. As a user, I want an application-wide default connection selection remembered as a multi-select, so that later installations need less repetition.
66a. As a user, I want the default connection selection to start with all four Agents checked, so that a normal install is immediately usable everywhere.
66b. As a user, I want to be able to deselect every Agent and install a pure stock Skill, so that content can enter the store before I decide where it lands.
66c. As a user installing a pure stock Skill, I want FanBox to write disable entries for Codex and ZCode, so that Agents that natively scan the store do not unexpectedly discover an unconnected Skill.
67. As a user, I want the confirmation page to always allow changing the remembered selection, so that the default never becomes a hidden decision.
68. As a user, I want FanBox to create the originals store directory only after installation confirmation, so that the choice is explicit.
...
71a. As a user, I want the installed original to live in the store and be referenced by links, config or copies rather than duplicated per Agent, so that one Skill has exactly one authoritative copy.
...
79a. As a user updating a Skill, I want the update to replace the original once via atomic swap, so that every connected Agent picks it up simultaneously without touching links or config.
79b. As a user updating a Skill, I want the confirmation to state how many Agent connections are affected, so that the blast radius is visible before I commit.
79c. As a user, I want a failed update to restore the previous original and its source record, so that connected Agents never see broken content.
...
83. As a user who edited a Skill original locally, I want FanBox to detect the content-hash mismatch, so that an update does not silently erase my changes.
84. As a user updating a locally modified original, I want explicit overwrite confirmation and the old content moved to Trash, so that my changes remain recoverable.
85. As a user with a store original that lacks a source record (a foreign original), I want FanBox to treat it as an unknown conflict, so that it does not infer identity from name alone.
86. As a user resolving an unknown same-name conflict, I want explicit confirmation before replacement and the old original moved to Trash with FanBox's source identity taking over, so that migration is deliberate, reversible and the original becomes updateable.
...
96. As a user, I want installation failures to leave no visible Skill original, so that the Installed tab remains trustworthy.
97. As a user, I want a successful installation to place the original in the store and connect the selected Agents through their native mechanisms, so that new Agent sessions can discover it.
97a. As a user, I want connections established at install time (links created, config entries written, WorkBuddy copies made) to behave exactly as if I had clicked each icon afterwards, so that there is one connection code path.
...
100. As a user, I want FanBox to switch to Installed after success and expand the new original row, so that I can immediately verify where it landed and which icons are lit.
...
107. As a maintainer, I want existing link, annex, uninstall and batch-management behavior to keep working, so that discovery does not regress the current Skills perspective.
...
- Use the project vocabulary consistently: an external searchable record is a **Skill entry**; **Discovery** searches and browses entries; **Installation** creates an **original** in the store and connects the selected Agents; promoting an off-store local copy into the store is **Annexation** (docs/14).
...
- Derive the target directory from the validated frontmatter name. Installation always targets the originals store `~/.agents/skills`; the installation API carries an optional multi-select list of the four supported Agent identifiers (Claude, Codex, ZCode, WorkBuddy), and the server resolves the store root and connection roots itself — it never accepts an arbitrary target path.
...
- Normal confirmation shows name, description, source identity, repository author, fixed commit short hash, connection selection (multi-select, default all four), Risk Check result, file count, expanded size and counts for scripts, binary resources, tools and dependencies. It offers complete file-list and source-page views.
...
- Remember an optional application-wide default connection selection in local settings, as a multi-select defaulting to all four Agents. Confirmation always exposes the selection and may create the missing store root only after confirmation. A pure stock install (no Agent selected) is legal: because Codex and ZCode natively scan the store, it writes disable entries for both so an unconnected original stays invisible to them.
...
- The same source identity at a different commit is a same-source reinstall/update. A same name with a different source identity is blocked and points to the existing original. An existing same-name original without a source record is a foreign original: explicit replacement takes over the slot, moves the old content to system Trash and records FanBox's source identity in its place (the takeover path, see docs/16).
- A same-source update replaces the original once: fixed-version re-extraction into a temporary sibling, impact notice stating the number of connected Agents, atomic swap, previous original to system Trash for rollback. Links and config are never touched during an update — every connected Agent picks up the new content immediately. WorkBuddy copies are refreshed per the drift rules in docs/16.
- Detect local edits by comparing the original tree with its recorded content hash. A locally modified original cannot be silently updated; replacement requires explicit confirmation and preserves the old content in system Trash.
...
- Install the original into the store, then connect each selected Agent through the single per-column connection path (Claude relative link, Codex config entry, ZCode config entry, WorkBuddy copy — ADR 0005); installation must not carry a private copy of connection logic. Do not restart Agents. On success, switch to Installed, preserve its filters, refresh data and expand the new original row; explain new-session and Codex-restart expectations.
...
- Prefer the highest existing backend seam: start a real FanBox service with an isolated HOME and isolated store plus Claude, Codex, ZCode and WorkBuddy roots; send HTTP requests through the public local API; assert normalized responses, filesystem results, source records, Trash/recovery effects and refreshed scan results. This extends the established Skills link, annex and batch-management integration-test pattern instead of introducing unit tests coupled to internal helpers.
...
- Cover successful public-GitHub installation into the store with every connection combination: all four Agents, a subset, and a pure stock install that leaves Codex and ZCode disable entries behind; include nested text, ordinary binary resources, empty directories and executable scripts with normalized modes.
...
- Cover target and conflict behavior: arbitrary target identifiers/paths are rejected; a missing store root can be created after confirmation; same-name different-source is blocked; same-source newer commit becomes an update whose confirmation reports the affected connection count; a foreign same-name original requires explicit takeover replacement; local content modification is detected.
...
- Cover post-install behavior: Skills data refreshes, the new original sits in the store with exactly the selected connections established through the per-column mechanisms, no restart is triggered, and the result reports restart/new-session guidance.
- Preserve existing behavior by running the current Skills link, annex, uninstall, batch and UI suites unchanged.
...
- Restoring copy-based cross-Agent import in any form (distribution is now per-row Agent links; see docs/16 and docs/14 for annexation).
...
- The architecture follows the current ADRs: Skill distribution uses the originals store with per-Agent connections (ADR 0001, rewritten); connection mechanisms per Agent are recorded in ADR 0005; external Installation is identified by repository plus Skill path and records a fixed commit/content hash (ADR 0003); external content comes from a commit-pinned GitHub archive rather than an unpinned skills.sh snapshot (ADR 0004).
