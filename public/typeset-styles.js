// 排版主题：从 huasheng_editor（editor.huasheng.ai）的 styles.js 原样搬来，只改了导出方式。
// 上游更新时可直接覆盖本文件，再把开头的 const STYLES 换成 window.TYPESET_STYLES 即可。
// 每套主题是一张「标签 → 内联样式」表；公众号编辑器只认内联样式，所以主题不能写成 CSS 类。
window.TYPESET_STYLES = {
  'wechat-default': {
    name: '默认公众号风格',
    styles: {
