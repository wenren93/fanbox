...
function createSkillDiscovery(options) {
  const {
    home, configDir, platform, queueWrite, trash, fingerprint,
    refreshSkills, readConfig, updateConfig,
  } = options;
  const supportedAgents = Array.isArray(options.supportedAgents) ? [...options.supportedAgents] : [];
  ...
  const diffSummary = options.diffSummary || (async () => null);
  ...
  async function inspect(body) {
    ...
    try {
      ...
      if (sameSource.length) {
        ...
        inspection.connectedAgents = row
          ? supportedAgents.filter((agent) => row.agents[agent] && row.agents[agent].on)
          : supportedAgents.filter((agent) => sameSource.some((record) => Array.isArray(record.agents) && record.agents.includes(agent)));
        inspection.affectedConnections = inspection.connectedAgents.length;
      }
  ...
  async function install(body) {
    ...
    return queueWrite(async () => {
      ...
      let targetHash = null;
      let unchanged = false;
      let connectionAgents = parsed.agents;
      let refreshWorkBuddy = false;
      ...
      try {
        ...
        const targetRow = (snapshot.items || []).find((item) => item.agents
          && (path.resolve(item.dir) === targetKey || (item.skillName || item.name) === inspection.name));
        ...
        if (targetHash && currentRecord) {
          ...
          // 同源更新只替换原件。接入范围取最终写入边界的现值，绝不采用确认页
          // 携带的 agents 重新配置各列；WorkBuddy 仅在原本已接入时按漂移规则重拷。
          connectionAgents = targetRow
            ? supportedAgents.filter((agent) => targetRow.agents[agent] && targetRow.agents[agent].on)
            : supportedAgents.filter((agent) => Array.isArray(currentRecord.agents) && currentRecord.agents.includes(agent));
          if (currentRecord.contentHash !== targetHash && !parsed.overwrite) return {
            ok: false, status: 'local_modified', error: '本机原件内容已修改，需要明确确认覆盖',
            targetHash, expectedTargetHash: targetHash, conflictFingerprint: targetHash,
            diff: await diffSummary(saved.skillDir, targetDir).catch(() => null),
          };
          ...
          if (!parsed.overwrite) return {
            ok: false, status: 'unknown_conflict', error: '同名原件没有 FanBox 来源记录，需要明确确认接管',
            targetHash, expectedTargetHash: targetHash, conflictFingerprint: targetHash, targetDir,
            diff: await diffSummary(saved.skillDir, targetDir).catch(() => null),
          };
        ...
        // 必须在原件换位前按旧原件判断 WB 漂移：相同副本与已落后副本可随原件刷新；
        // WB 抢先编辑（内容不同且 mtime 更新）保留，交给用户后续裁决。
        const workBuddyState = targetRow && targetRow.agents && targetRow.agents.workbuddy;
        refreshWorkBuddy = Boolean(targetHash && !unchanged && workBuddyState && workBuddyState.on
          && (workBuddyState.matchesOriginal || workBuddyState.drift));
        if (targetHash && !unchanged && workBuddyState && workBuddyState.on && connectionAgents.includes('workbuddy')) {
          workBuddyStatus = refreshWorkBuddy ? 'refreshed' : 'preserved_ahead';
        }
        ...
        connections = await applyConnections(inspection.name, connectionAgents, {
          refreshWorkBuddy,
        });
        if (!connections.ok) {
        ...
        const record = {
          ...
          contentHash: inspection.contentHash, name: inspection.name, agents: connectionAgents,
        ...
        if (rollback) {
          const previousContentTag = currentRecord && currentRecord.contentHash === targetHash
            && /^[a-f0-9]{40}$/i.test(currentRecord.commit || '')
            ? currentRecord.commit.slice(0, 12)
            : `${currentRecord ? 'modified' : 'foreign'}-${targetHash.slice(0, 12)}`;
          const recoveryName = `${inspection.name}-previous-${previousContentTag}-${Date.now()}`;
          const recoveryDir = path.join(storeRoot, recoveryName);
          await fsp.rename(rollback, recoveryDir);
          rollback = recoveryDir;
          const result = await trash(rollback);
          if (!result.ok) throw new Error(result.error || '旧安装项移入废纸篓失败');
          recovery = { name: recoveryName, trashPath: result.destination || null };
          rollback = null;
        }
        ...
        return {
          ok: true, status: unchanged ? 'identical' : targetHash ? 'updated' : 'installed', agents: connectionAgents,
          targetDir: visibleTargetDir, item: item || null, installation: record,
          restartRequired, recovery, workBuddyStatus,
          message: workBuddyStatus === 'preserved_ahead'
            ? '原件已更新；现有接入保持不变；检测到 WorkBuddy 抢先编辑，已保留该拷贝且未刷新，其余接入已即时生效'
            : targetHash && !unchanged
            ? '原件已更新；现有接入保持不变，内容已即时生效；旧原件可从系统废纸篓恢复'
            : !connectionAgents.length
