  ...
  async function install(body) {
    ...
    return queueWrite(async () => {
      ...
      try {
        ...
        if (rollback) {
          const priorCommit = currentRecord && currentRecord.contentHash === targetHash
            && /^[a-f0-9]{40}$/i.test(currentRecord.commit || '')
            ? currentRecord.commit.slice(0, 12)
            : `${currentRecord ? 'modified' : 'foreign'}-${targetHash.slice(0, 12)}`;
          const recoveryName = `${inspection.name}-previous-${priorCommit}-${Date.now()}`;
        ...
        return {
          ...
          restartRequired, recovery,
          message: targetHash && !unchanged
