...
function createSkillDiscovery(options) {
  ...
  async function inspect(body) {
    ...
    try {
      ...
      const inspection = {
        ...
        targets: Object.values(targets).map(({ id: targetId, label }) => ({ id: targetId, label })),
      };
  ...
  function parseInstall(body) {
    ...
    if (typeof body.targetAgent !== 'string' || !Object.prototype.hasOwnProperty.call(targets, body.targetAgent)) return { ok: false, error: '未知的目标 Agent' };
    ...
    return { ok: true, inspectionId: body.inspectionId, targetAgent: body.targetAgent, acknowledge: body.acknowledge === true, overwrite: body.overwrite === true, expectedTargetHash: expectedTargetHash || null };
  }

  async function install(body) {
    const parsed = parseInstall(body);
    ...
    if (inspection.enhancedConfirmation && !parsed.acknowledge) return { ok: false, status: 'acknowledgement_required', error: '请展开风险明细并确认后再安装' };
    const target = targets[parsed.targetAgent];
    const targetDir = path.join(target.root, inspection.name);
    return queueWrite(async () => {
      ...
      try {
        ...
        const currentRecord = records.installations[path.resolve(targetDir)] || null;
        const snapshot = await refreshSkills();
        const targetRoot = path.resolve(target.root);
        const disabledRoot = target.id === 'workbuddy' ? path.resolve(path.dirname(target.root), 'skills_disabled') : null;
        const nameConflict = (snapshot.items || []).find((item) => {
          if (item.residue || path.resolve(item.dir) === path.resolve(targetDir)) return false;
          const itemDir = path.resolve(item.dir);
          const inside = itemDir.startsWith(`${targetRoot}${path.sep}`)
            || (disabledRoot && itemDir.startsWith(`${disabledRoot}${path.sep}`));
          return inside && (item.skillName || item.name) === inspection.name;
        });
        if (nameConflict) {
          return {
            ok: false, status: 'different_source_conflict',
            error: '目标 Agent 已有同名 Skill 安装项；FanBox 不会凭名称推断来源或创建重复项',
        ...
        if (targetHash && currentRecord) {
          ...
          if (existingIdentity !== identity) return { ok: false, status: 'different_source_conflict', error: '同名安装项来自不同来源，不能按名称覆盖', conflict: { targetDir, record: currentRecord } };
          if (currentRecord.contentHash !== targetHash && !parsed.overwrite) return { ok: false, status: 'local_modified', error: '本机安装内容已修改，需要明确确认覆盖', targetHash, expectedTargetHash: targetHash, conflictFingerprint: targetHash };
          ...
          if (currentRecord.commit === inspection.commit && targetHash === inspection.contentHash) return { ok: true, status: 'identical', targetAgent: target.id, targetDir, installation: currentRecord };
        } else if (targetHash && !currentRecord) {
          if (!parsed.overwrite) return { ok: false, status: 'unknown_conflict', error: '同名目录没有 FanBox 来源记录，需要明确确认覆盖', targetHash, expectedTargetHash: targetHash, conflictFingerprint: targetHash, targetDir };
        ...
        await fsp.mkdir(target.root, { recursive: true, mode: 0o755 });
        stage = await fsp.mkdtemp(path.join(target.root, '.fanbox-discovery-stage-'));
        ...
        if (targetHash) {
          rollback = await fsp.mkdtemp(path.join(target.root, '.fanbox-discovery-rollback-'));
        ...
        const record = {
          ...
          contentHash: inspection.contentHash, name: inspection.name, targetAgent: target.id,
          installedAt: Date.now(), sourceUrl: inspection.sourceUrl, discoveryEntryId: inspection.discoveryEntryId,
        };
        records.installations[path.resolve(targetDir)] = record;
        ...
        const item = (refreshed.items || []).find((it) => path.resolve(it.dir) === path.resolve(targetDir));
        return {
          ok: true, status: targetHash ? 'updated' : 'installed', targetAgent: target.id, targetLabel: target.label,
          targetDir, item: item || null, installation: record,
          restartRequired: target.id === 'codex' ? 'codex' : null,
          message: target.id === 'codex' ? '新会话可发现此 Skill；Codex 可能需要重启' : '新 Agent 会话可发现此 Skill',
        };
      } catch (error) {
        ...
        } else if (installedVisible && oldRecords && !oldRecords.installations[path.resolve(targetDir)]) {
        ...
        return {
          ok: false,
          status: rollbackErrors.length ? 'rollback_failed' : 'install_failed',
  ...
  async function settings(body) {
    if (body === undefined) {
      ...
      const value = Object.prototype.hasOwnProperty.call(targets, config.defaultSkillTargetAgent) ? config.defaultSkillTargetAgent : 'codex';
      return {
        ok: true,
        defaultTargetAgent: value,
        targets: Object.values(targets).map(({ id, label }) => ({ id, label })),
        installationPrerequisite: await prerequisites(),
      };
    }
    const value = body && body.defaultTargetAgent;
    if (!Object.prototype.hasOwnProperty.call(targets, value)) return { ok: false, status: 'invalid_request', error: '未知的默认目标 Agent' };
    await updateConfig((config) => { config.defaultSkillTargetAgent = value; });
    return { ok: true, defaultTargetAgent: value };
  }

  async function sourceRecord(dir) {
    const key = path.resolve(dir);
    const records = await loadRecords();
    return records.installations[key] || null;
  }

  async function removeSourceRecord(dir) {
    const key = path.resolve(dir);
    const records = await loadRecords();
    if (!records.installations[key]) return null;
    const removed = records.installations[key];
  ...
  async function restoreSourceRecord(dir, record) {
    ...
    const records = await loadRecords();
    records.installations[path.resolve(dir)] = record;
...
module.exports = { createSkillDiscovery, normalizeSearchPayload, parseGitHubSource };
