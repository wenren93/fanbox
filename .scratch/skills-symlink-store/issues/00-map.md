...
- **术语定稿（2026-08-23，07 号票会话）**：「正本」统一改为「**原件**」（English: original）；本地图与旧决议行中的「正本」即今「原件」，产品文档已全量替换。
- **实施交接（更新）**：决策票全部收敛、规格已成文（docs/16 总纲 + docs/14 收编 + docs/15 安装增补 + ADR 0001/0005）。实施票 #20–#29（ready-for-agent）已挂本地图下，阻塞边写在各票头部；实施会话先读 docs/16 与对应规格票，用 implement/tdd 技能执行。
...
- [07 — 规格成文：docs 14/15 改写 + ADR 0001 改写 + CONTEXT.md 定稿](https://github.com/wenren93/fanbox/issues/18)（本地: issues/07-spec-and-adr-rewrite.md）: 七项清单全交付——ADR 0001 改写（保留决策轨迹）+ 新增 ADR 0005（各 Agent 接入与启停机制）；docs/14 改为「收编为原件规格」；docs/15 增补原件仓安装/多选接入/同源更新新语义；新总纲 docs/16（模型/接口/UI/外来原件/迁移 Phase 0–5/验收八条）；CONTEXT.md·CLAUDE.md·README 定稿；**术语「正本」→「原件」**；实施拆票 #20–#29 发布为地图子票。决议: [issuecomment-5384629515](https://github.com/wenren93/fanbox/issues/18#issuecomment-5384629515)
...
---
**本票即地图**；子票以 GitHub sub-issue 挂在本票下，开放票用 `is:issue is:open label:^wayfinder` 查询。本地镜像: [.scratch/skills-symlink-store/issues/](https://github.com/wenren93/fanbox/tree/feature/.scratch/skills-symlink-store/issues)
